...
export const PE_SYSTEM_PROMPTS: Record<string, string> = {
  ...
  chroma: peSys(
    "MODEL: FLUX / Chroma (image). Write each prompt as a single flowing natural-language description, NOT a comma-separated tag list. Order it: subject → action/pose → setting → lighting → composition. CRITICAL: Chroma is NOT focused on realism. Do NOT use any photorealistic tags, camera models, lenses, or brands (no '85mm', 'shot on Fuji', 'photorealistic', 'visible pores'). Write as though it's for Flux, but stylized, cinematic, or illustrated. Aim for 30-80 words. Avoid contradictory directions."
  ),
  zimage: peSys(
